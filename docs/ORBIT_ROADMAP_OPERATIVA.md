...
| **Fase 9**  | Testing & tuning        | Validazione UX, IA, performance                                        | ✅ Completo | `useOrbitInhibition` hook + ViewManager wiring + 36 unit tests ✅ + E2E 8 scenari + `useWorldStore.test.ts` 24/24 ✅ + `OrbitControlPanel` settings UI + `OrbitControlPanel.test.tsx` 34/34 ✅ + Settings.tsx wiring + **Lighthouse 98/100** (LCP 1.3s, TBT 160ms) |
...
### Sprint J — Lighthouse ≥ 90 / Performance (Fase 9 completamento) ✅

- **Baseline**: Performance 70/100, LCP 5.8s (92% Render Delay), TBT 360ms
- **Ottimizzazioni**:
  - `src/stores/lazyStores.ts` — `preloadAllStores()` ora usa `Promise.allSettled` (tutti i 5 store in parallelo invece che sequenziali)
  - `src/main.tsx` — 5 dynamic imports in `bootstrapApp()` ora sono `Promise` parallele avviate prima di `await preloadAllStores()`
  - `src/components/App.tsx` — `FloatingSatelliteCopilot` e `ModalManager` ora `React.lazy()` (rimosso da critical path, riduce App chunk 306 KB → 205 KB, -33%)
  - `vite.config.ts` — `build.target: 'esnext'` (rimuove legacy polyfills ~11 KB); plugin inline `preload-app-chunk` inietta `<link rel="modulepreload">` per App e Home in `index.html`
  - `index.html` — testo LCP ("Sono DocenteDoc AI… assistente intelligente per la didattica") inline nella splash screen HTML — LCP visibile prima di qualsiasi JS
- **Risultato**: **Performance 98/100**, LCP 1.3s ✅, FCP 1.3s ✅, TBT 160ms ✅, CLS 0.001 ✅
