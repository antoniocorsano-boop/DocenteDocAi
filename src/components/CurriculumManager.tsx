...
const CurriculumManager: React.FC<CurriculumManagerProps> = ({ curricula, onUpdateCurricula, settings, aiSettings, onNavigate }) => {
    ...
    const renderEditor = () => {
        ...
        return (
            <div className="flex-grow overflow-y-auto p-6 space-y-8 bg-surface-container-lowest/50 backdrop-blur-sm custom-scrollbar">
                ...
                {selectedCurriculum.nuclei.map((nucleo, nIdx) => (
                    <InfoCard 
                        key={nucleo.id} 
                        className="border-l-4 border-l-primary bg-surface-container-high/30 backdrop-blur-md shadow-sm hover:shadow-md transition-shadow"
                    >
                            ...
                            {nucleo.objectives.map((obj, oIdx) => (
                                <div key={obj.id} className="flex gap-2 items-center group">
                                    ...
                                    <M3Button 
                                        ...
                                        variant="text"
                                        className="opacity-0 group-hover:opacity-100 !w-8 !h-8 text-on-surface-variant hover:bg-error-container hover:text-on-error-container !min-w-0 !p-0"
                                    >
                ...
                <M3Button 
                    ...
                    variant="outlined"
                    className="w-full border-dashed !rounded-2xl py-8 border-2 font-extrabold hover:bg-primary/5 transition-colors"
                >
        ...
        <div className="page-layout h-[calc(100vh-64px)] overflow-hidden !gap-0 !p-0 md:!p-4">
            <div className="flex h-full bg-surface-container-low/30 backdrop-blur-xl md:rounded-4xl overflow-hidden border border-outline-variant/30 shadow-2xl">
                <div className="w-80 border-r border-outline-variant/30 flex flex-col bg-surface-container-low/50 backdrop-blur-md flex-shrink-0">
                    ...
                    <div className="p-6 border-b border-outline-variant/30 bg-surface-container-high/20 space-y-5">
                        ...
                        <M3Button onClick={handleCreate} variant="filled" className="w-full font-black shadow-lg py-4">
                            Crea Curricolo
                        </M3Button>
                    </div>
