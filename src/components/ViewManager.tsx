                    ...
                    if (config) {
                        ...
                        switch (view) {
                            ...
                            case 'orientamento':
                                ...
                                break;
                            case 'lessons':
                                componentProps = { lessons: Object.values(lessons), uda, knowledgeBase, userClasses: settings.classi, onViewLesson: setLessonViewContext as (lesson: unknown) => void ?? (() => {}), onAddLessons, onUpdateLesson: (lesson: Lezione) => setLessons(prev => ({ ...prev, [lesson.id]: lesson })), aiSettings, setIsLoadingModalOpen, setLoadingModalMessage, slots, onScheduleLesson, curricula, settings, onStartClassroom: () => {}, initialUdaId: typeof viewContext === 'object' && viewContext !== null && 'udaId' in viewContext ? String((viewContext as Record<string, unknown>).udaId) : undefined, initialClass: typeof viewContext === 'string' ? viewContext : undefined };
                                break;
                            case 'uda':
