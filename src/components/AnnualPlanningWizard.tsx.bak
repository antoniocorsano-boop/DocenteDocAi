...
const AnnualPlanningWizard: React.FC<AnnualPlanningWizardProps> = ({
  ...
  const [step, setStep] = useState<WizardStep>('context');
    ...
    const renderStepIndicator = () => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--app-spacing-container)', padding: 'var(--app-spacing-container)' }} role="navigation" aria-label="Progressi del wizard">
            {['Contesto', 'Analisi', 'Metodi', 'Piano', 'Anteprima', 'Output'].map((label, idx) => {
                ...
                return (
                    <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--app-spacing-component)' }} aria-current={isActive ? 'step' : undefined}>
                        <div
                            style={{
                        ...
                        <span style={{ fontSize: 'var(--md-sys-typescale-body-small-size)', textAlign: 'center', color: 'var(--app-color-on-surface)' }}>
                        ...
                        {idx < 5 && (
                            <div
                                style={{
                                    ...
                                    height: 'var(--app-border-thick)',
                                    backgroundColor: isDone ? 'var(--app-color-primary)' : 'var(--md-sys-color-outline-variant)',
            ...
            <M3DialogContent>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--app-spacing-container)' }}>
                            <div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--app-spacing-container)' }}>
                                    <div>
                                        ...
                                        <select id="wizard-select-class" name="wizard-select-class" value={selectedClass} onChange={e => setSelectedClass(e.target.value)} style={{ width: 'var(--app-layout-full)' }} title="Seleziona la classe per la programmazione">
                                            {userClasses.map(c => <option key={c} value={c}>{c}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label htmlFor="wizard-select-subject">Materia</label>
                                        <select id="wizard-select-subject" name="wizard-select-subject" value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} style={{ width: 'var(--app-layout-full)' }} title="Seleziona la materia">
                            ...
                            <div style={{ backgroundColor: 'var(--md-sys-color-surface-container-low)', borderRadius: 'var(--md-sys-shape-corner-large)', padding: 'var(--md-sys-spacing-8)', border: `var(--app-border-normal) solid var(--md-sys-color-outline)` }}>
                                <h4 style={{ marginBottom: 'var(--md-sys-spacing-8)', display: "flex", alignItems: "center", gap: 'var(--md-sys-spacing-8)' }}>
                                    <span style={{ fontFamily: 'Material Symbols Outlined', color: 'var(--app-color-secondary)' }}>folder_open</span>
                                    ...
                                    {recommendedFiles.length > 0 ? recommendedFiles.map(kb => (
                                        <div key={kb.id} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', padding: 'var(--app-spacing-component)' }}>
                                            ...
                                            <label htmlFor={`kb-annual-${kb.id}`} style={{ display: 'flex', flexDirection: 'row', width: 'var(--md-sys-percent-full)', justifyContent: 'flex-start', cursor: 'pointer' }} title={kb.fileName}>
                                                ...
                                                <span style={{ fontFamily: 'Material Symbols Outlined', color: 'var(--app-color-primary)', marginRight: "var(--app-spacing-component)" }}>description</span>
                    ...
                    {step === 'situation' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--app-spacing-container)' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 'var(--app-spacing-component)' }}>
                    ...
                    {step === 'methodology' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--app-spacing-container)' }}>
                            ...
                            <div style={{ backgroundColor: 'var(--app-color-secondary-container)', borderRadius: 'var(--md-sys-shape-corner-large)', padding: 'var(--md-sys-spacing-8)', border: `var(--app-border-thin) solid var(--md-sys-color-outline)` }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 'var(--md-sys-spacing-8)' }}>
                                    ...
                                    <M3Button variant="text" onClick={handleGenerateMethodology} disabled={!!methodologyStatus} style={{ display: 'flex', flexDirection: 'row', alignItems: "center", gap: 'var(--md-sys-spacing-8)' }} title="Suggerisci metodologie adatte al contesto">
                                        {methodologyStatus ? <AiThinkingGem size="small" inline text="Thinking..." /> : <><span style={{ fontFamily: 'Material Symbols Outlined', color: 'var(--app-color-primary)' }}>lightbulb</span> Suggerisci</>}
                                    </M3Button>
                                </div>
                    ...
                    {step === 'sequence' && (
                        <div style={{ gap: 'var(--app-spacing-container)' }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div style={{ display: 'flex', flexDirection: 'row', alignItems: "center", gap: 'var(--md-sys-spacing-8)' }}>
                                    ...
                                    <button
                                        ...
                                        style={{ color: "var(--app-color-secondary)", background: 'none', border: 'none', cursor: 'pointer' }}
                                ...
                                <div style={{ display: 'flex', flexDirection: 'row', gap: 'var(--app-spacing-component)' }}>
                                    <div style={{ backgroundColor: 'var(--md-sys-color-surface-container-high)', borderRadius: 'var(--md-sys-shape-corner-large)', display: "flex", alignItems: "center", gap: 'var(--md-sys-spacing-8)', padding: 'var(--app-spacing-component)', border: "var(--app-border-thin) solid var(--md-sys-color-outline)" }}>
                                        <label htmlFor="wizard-hours-per-week">Ore/Sett:</label>
                                        <input id="wizard-hours-per-week" name="wizard-hours-per-week" type="number" value={hoursPerWeek} onChange={e => setHoursPerWeek(Math.max(1, parseInt(e.target.value)))} style={{ width: "var(--md-sys-spacing-10)", backgroundColor: "transparent", textAlign: "center", fontWeight: "bold", borderBottom: "var(--app-border-thin) solid var(--md-sys-color-outline)" }} title="Ore settimanali di lezione" />
                                    </div>
                            ...
                            <div style={{ backgroundColor: 'var(--md-sys-color-surface-container-high)', borderRadius: 'var(--md-sys-shape-corner-large)', display: "flex", gap: 'var(--md-sys-spacing-8)', alignItems: "flex-end", marginBottom: 'var(--md-sys-spacing-8)', padding: 'var(--app-spacing-section)', border: "var(--app-border-thin) solid var(--md-sys-color-outline)" }}>
                                ...
                                <M3Button variant="filled" onClick={addUdaToPlan} style={{ marginBottom: 'var(--app-spacing-container)' }} title="Aggiungi alla lista">Aggiungi</M3Button>
                            </div>
                                        ...
                                        <div key={uda.id} style={{ backgroundColor: 'var(--md-sys-color-surface-container-high)', borderRadius: 'var(--md-sys-shape-corner-large)', display: "flex", alignItems: "center", gap: 'var(--app-spacing-section)', padding: 'var(--app-spacing-section)', border: "var(--app-border-thin) solid var(--md-sys-color-outline)" }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                                                <div style={{ display: 'flex', flexDirection: 'row', alignItems: "center", gap: 'var(--md-sys-spacing-8)', marginBottom: 'var(--app-spacing-container)' }}>
                                                    <span style={{ fontWeight: "bold", backgroundColor: "var(--app-color-primary)", color: "var(--app-color-on-primary)", paddingLeft: 'var(--app-spacing-container)', paddingRight: 'var(--app-spacing-container)', borderRadius: 'var(--app-spacing-container)' }}>
                                                        UDA {idx + 1}
                                                    </span>
                    ...
                    {step === 'preview' && (
                        <div style={{ gap: 'var(--app-spacing-container)' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 'var(--app-spacing-section)' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                            ...
                            <div style={{ gap: 'var(--app-spacing-section)', paddingTop: 'var(--app-spacing-container)', paddingBottom: 'var(--app-spacing-container)', overflowY: "auto", maxHeight: 'var(--md-sys-spacing-24)' }}>
                                {schedulePreview.map((item, idx) => (
                                    <div key={idx} style={{ position: "relative", paddingLeft: 'var(--app-spacing-section)' }}>
                    ...
                    {step === 'document' && (
                        <div style={{ gap: 'var(--app-spacing-section)', display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "var(--md-sys-percent-full)", textAlign: "center" }}>
                            <div style={{ color: 'var(--app-color-on-secondary-container)', width: 'var(--app-spacing-container)', height: 'var(--app-spacing-container)', borderRadius: 'var(--app-spacing-container)', backgroundColor: 'var(--app-color-secondary)', display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 'var(--md-sys-spacing-8)' }}>
                                <span style={{ fontFamily: 'Material Symbols Outlined', color: 'var(--app-color-on-secondary-container)' }}>check_circle</span>
                            </div>
                            <h3 style={{ color: 'var(--app-color-on-surface)' }}>Pianificazione Completata!</h3>
