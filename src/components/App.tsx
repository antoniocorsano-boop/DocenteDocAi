...
const CircolareAnalysisModal = React.lazy(
  () => import("./CircolareAnalysisModal"),
);
...
const App: React.FC = () => {
  ...
  const [assistantMode] = React.useState<"chat" | "docs" | "tools" | "backup">(
    "chat",
  );
  ...
  const [circularAnalysisPayload, setCircularAnalysisPayload] = React.useState<{
    url: string;
    title: string;
  } | null>(null);
    ...
    <>
      ...
      >
        <ErrorBoundary>
          ...
          {openModal === "circular-analysis-modal" &&
            circularAnalysisPayload && (
              <React.Suspense fallback={<div>Loading...</div>}>
