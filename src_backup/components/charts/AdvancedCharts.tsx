...
export const LineChart: React.FC<LineChartProps> = ({ data, color, height = 250 }) => {
  const { layers } = useTheme();
    ...
    if (data.length === 0) {
        return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", opacity: "0.5", color: layers.sys.colors.onSurfaceVariant }}>Nessun dato disponibile.</div>;
    }
    ...
    return (
        <div  style={{ width: "100%", height }}>
            <svg viewBox={`0 0 ${width} ${height}`}  style={{ width: "100%", height: "100%" }}>
                {/* Grids */}
                <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke={layers.sys.colors.outlineVariant} strokeWidth="1" />
                <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke={layers.sys.colors.outlineVariant} strokeWidth="1" />
                ...
                {data.map((point, i) => (
                    <circle
                        ...
                        r="4"
                        fill={color}
                        stroke={layers.sys.colors.surface}
                        strokeWidth="2"
                    />
                ))}
                ...
                {data.map((point, i) => (
                    (data.length < 8 || i === 0 || i === data.length - 1 || i % Math.ceil(data.length / 5) === 0) && (
                        <text key={i} x={getX(i)} y={height - 5} fontSize="10" textAnchor="middle" fill={layers.sys.colors.onSurfaceVariant}>
                            {point.label}
                        </text>
                    )
                ))}
...
export const RadarChart: React.FC<RadarChartProps> = ({ data, color, size = 300 }) => {
    ...
    if (data.length === 0) {
        return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: ref.spacing[300], opacity: "0.5", color: layers.sys.colors.onSurfaceVariant }}>Dati competenze non disponibili.</div>;
    }
            ...
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} >
                ...
                {levels.map(level => (
                    <polygon
                        ...
                        fill="none"
                        stroke={layers.sys.colors.outlineVariant}
                        strokeWidth="1"
                        strokeDasharray="4 4"
                    />
                ))}
                ...
                {data.map((_, i) => {
                    ...
                    return <line key={i} x1={center} y1={center} x2={x} y2={y} stroke={layers.sys.colors.outlineVariant} strokeWidth="1" />;
                })}
                    ...
                    return (
                        <g key={i}>
                            ...
                            <text
                                ...
                                fontSize="10"
                                fill={layers.sys.colors.onSurface}
