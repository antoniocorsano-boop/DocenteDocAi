...
vi.mock('./ui', () => ({
  ...
  M3ExpressiveCard: ({ title, description, children }: any) => (
    <div style={{backgroundColor: 'layers.sys.colors.surface-container-high', borderRadius: 'layers.ref.shape.corner.large'}}>
      <div>{title}</div>
      <div>{description}</div>
      {children}
    </div>
  ...
  M3Button: ({ children, onClick, variant, 'aria-label': ariaLabel, ...props }: any) => (
    <button onClick={onClick} data-variant={variant} aria-label={ariaLabel} {...props} style={{color: 'layers.sys.colors.primary'}}>
      {children}
    </button>
  ),
