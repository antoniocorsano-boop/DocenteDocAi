  ...
  const { layers } = useTheme();
    ...
    return (
        <section className={`bg-[var(--md-sys-color-surface-container-low)] rounded-[var(--md-sys-shape-corner-extra-large)] border border-[var(--md-sys-color-outline-variant)] overflow-hidden mb-6 shadow-[var(--md-sys-elevation-level1)] ${className}`}>
            <div style={{ backgroundColor: sys.colors.surface/50 }} style={{display: "flex", alignItems: "center", gap: layers.ref.spacing['8'], padding: layers.ref.spacing['5'], borderBottom: "1px solid layers.sys.colors.outline"}}>
                ...
                <div style={{ flexGrow: "1", minWidth: "0" }}>
                    ...
                    {subtitle && <p style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ opacity: "0.8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{subtitle}</p>}
                </div>
            </div>
