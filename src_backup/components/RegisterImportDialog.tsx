                ...
                {step === 'upload' && (
                    <>
                        <div style={{marginTop: layers.ref.spacing['4']}}>
                            ...
                            <div style={{ borderRadius: ref.shape[], backgroundColor: sys.colors.secondary-container/30 }} style={{padding: layers.ref.spacing['8'], border: "1px solid layers.sys.colors.outline", display: "flex", gap: layers.ref.spacing['6']}}>
                                <span  style={{color: "layers.sys.colors.secondary"}}>info</span>
                        ...
                        >
                            ...
                            <div style={{ textAlign: "center" }}>
                                ...
                                <p style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }}>Supporta .csv, .xlsx, .xls</p>
                            </div>
                        </div>

                        {error && (
                            <div style={{ borderRadius: ref.shape[], color: sys.colors.on-error-container }} style={{padding: layers.ref.spacing['8'], backgroundColor: "layers.sys.colors.error-container", display: "flex", alignItems: "center", gap: layers.ref.spacing['6']}}>
                                <span style={{
  fontFamily: 'Material Symbols Outlined'
}}>error</span>
                                <p style={{ color: sys.colors.[var(--md-sys-typescale-body-medium)] }}>{error}</p>
                            </div>
                        )}
                    </>
                )}
                ...
                {step === 'mapping' && rawData && (
                    <div  style={{gap: layers.ref.spacing['6']}}>
                                ...
                                <div style={{gap: layers.ref.spacing['4'], padding: layers.ref.spacing['8']}}>
                                    <div>
                                        ...
                                        <select 
                                            ...
                                            style={{ borderRadius: ref.shape[] }} style={{width: "100%", padding: layers.ref.spacing['8'], border: "1px solid layers.sys.colors.outline", backgroundColor: "layers.sys.colors.surface", fontSize: "0.875rem"}}
                                        >
                        ...
                        <div style={{ borderRadius: ref.shape[], backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)] }} style={{padding: layers.ref.spacing['8'], overflowX: "auto"}}>
                            <p style={{ color: sys.colors.[10px] }} style={{fontWeight: "bold", textTransform: "uppercase", letterSpacing: "0.1em", opacity: "0.5", marginBottom: layers.ref.spacing['8']}}>Anteprima Dati Raw (Prime 3 righe)</p>
                            <table style={{ color: sys.colors.[10px] }} style={{ width: "100%" }}>
                                <thead>
                                    <tr>
                                        {rawData.headers.map(h => <th key={h}  style={{border: "1px solid layers.sys.colors.outline", padding: layers.ref.spacing['1'], textAlign: "left", backgroundColor: "layers.sys.colors.surface"}}>{h}</th>)}
                                    </tr>
                                </thead>
                                <tbody>
                                    {rawData.data.slice(0, 3).map((row, i) => (
                                        <tr key={i}>
                                            {rawData.headers.map(h => <td key={h}  style={{border: "1px solid layers.sys.colors.outline", padding: layers.ref.spacing['1']}}>{String(row[h] || '')}</td>)}
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                {step === 'preview' && result && (
                    <div  style={{gap: layers.ref.spacing['4']}}>
                        <div style={{display: "flex", alignItems: "center", gap: layers.ref.spacing['6'], color: "layers.sys.colors.primary"}}>
                            <span style={{ color: sys.colors.3xl }}>check_circle</span>
                        ...
                        <div  style={{display: "grid", gridTemplateColumns: "1fr", gap: layers.ref.spacing['8']}}>
                            <InfoCard title="Riepilogo" icon="analytics">
                                <ul style={{gap: layers.ref.spacing['2']}}>
                                    <li style={{ color: sys.colors.[var(--md-sys-typescale-body-medium)] }} style={{ display: "flex", justifyContent: "space-between" }}>
                                        <span>Studenti:</span>
                            ...
                            <InfoCard title="Classi rilevate" icon="class" variant="secondary">
                                <div style={{display: "flex", flexWrap: "wrap", gap: layers.ref.spacing['8']}}>
                                    {Array.from(new Set(result.students.map(s => s.classe))).map(c => (
                                        <span key={c} style={{ color: sys.colors.on-secondary-container }} style={{paddingLeft: layers.ref.spacing['4'], paddingRight: layers.ref.spacing['4'], borderRadius: "0.375rem", backgroundColor: "layers.sys.colors.secondary-container", fontWeight: "bold"}}>
                                            {c}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {result.errors.length > 0 && (
                            <div style={{ borderRadius: ref.shape[], backgroundColor: sys.colors.warning-container, color: sys.colors.on-warning-container }} style={{padding: layers.ref.spacing['8']}}>
