...
const MaterialPickerModal: React.FC<MaterialPickerModalProps> = ({ knowledgeBase, currentMaterials, onClose, onSave }) => {
  const { layers } = useTheme();
    ...
    return (
        <M3Dialog
            ...
            level={2}
        >
            <M3DialogContent style={{ padding: ref.spacing[0], backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)]/30 }}>
                <div  style={{ display: "grid", gridTemplateColumns: "1fr" }}>
                    {/* Left: Source */}
                    <div  style={{padding: layers.ref.spacing['6'], borderRight: "1px solid layers.sys.colors.outline", display: "flex", flexDirection: "column", gap: layers.ref.spacing['6']}}>
                        <TabGroup
                            tabs={tabs}
                            activeTab={activeTab}
  ...
  fontFamily: 'Material Symbols Outlined'
                        ...
                        {activeTab === 'file' && (
                            ...
                            >
                                <input {...getInputProps()} />
                                <div style={{ backgroundColor: sys.colors.primary/10 }} style={{width: ref.spacing[64], height: ref.spacing[64], borderRadius: ref.spacing[9999], display: "flex", alignItems: "center", justifyContent: "center", marginBottom: layers.ref.spacing['8']}}>
                                    <span style={{ color: sys.colors.3xl }} style={{color: "layers.sys.colors.primary"}}>upload_file</span>
                                </div>
                    ...
                    {/* Right: Selected */}
                    <div style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-low)]est/50 }} style={{padding: layers.ref.spacing['6'], display: "flex", flexDirection: "column", gap: layers.ref.spacing['8']}}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <h3 >Selezionati</h3>
                            <span  style={{borderRadius: ref.spacing[9999], backgroundColor: "layers.sys.colors.primary", color: "layers.sys.colors.on-primary", fontSize: "0.75rem", fontWeight: "bold"}}>
                                {materials.length}
                            </span>
                        </div>
                        <div  style={{flexGrow: "1", overflowY: "auto", gap: layers.ref.spacing['2']}}>
                            {materials.map(material => (
                                <div key={material.id} style={{ borderRadius: ref.shape[], backgroundColor: sys.colors.[var(--md-sys-color-surface-container)] }} style={{display: "flex", alignItems: "center", justifyContent: "space-between", padding: layers.ref.spacing['6'], border: "1px solid layers.sys.colors.outline"}}>
                                    <div style={{display: "flex", alignItems: "center", gap: layers.ref.spacing['6'], overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap"}}>
                                        <span  style={{color: "layers.sys.colors.primary"}}>{getMaterialIcon(material)}</span>
                                    ...
                                    <M3Button 
                                        ...
                                        variant="text" 
                                         style={{color: "layers.sys.colors.error", opacity: "0", transition: "opacity 300ms"}}
                                    >
                                        <span style={{
  fontFamily: 'Material Symbols Outlined'
}}>close</span>
