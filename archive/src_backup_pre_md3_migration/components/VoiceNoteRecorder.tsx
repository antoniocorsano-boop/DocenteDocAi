...
const VoiceNoteRecorder: React.FC<VoiceNoteRecorderProps> = ({ onTranscription, compact = false }) => {
  const { layers } = useTheme();
    ...
    return (
        <button
            ...
            style={{display: 'flex',
                ...
                backgroundColor: isRecording 
                    ? layers.sys.colors.errorContainer 
                    : layers.sys.colors.secondaryContainer,
                color: isRecording 
                    ? layers.sys.colors.onErrorContainer 
                    : layers.sys.colors.onSecondaryContainer,
                ...
                position: 'relative',
                overflow: 'hidden',
                minWidth: compact ? ref.spacing[48] : 'auto',
                height: compact ? ref.spacing[48] : 'auto',
        ...
        >
            {isProcessing ? (
                <div style={{width: ref.spacing[20],
                    height: ref.spacing[20],
                    border: '2px solid layers.sys.colors.outline',
                    borderTop: '2px solid layers.sys.colors.primary',
                    borderRadius: '50%',
                    animation: 'spin 1s linear infinite'}} />
            ) : (
                <span style={{fontFamily: 'Material Symbols Outlined',
                    fontSize: compact ? ref.spacing[20] : ref.spacing[24],
