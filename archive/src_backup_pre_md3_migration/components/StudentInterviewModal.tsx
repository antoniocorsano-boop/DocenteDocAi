            ...
            <M3DialogContent style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)]/30 }}>
                <div  style={{display: "grid", gridTemplateColumns: "1fr", gap: layers.ref.spacing['6'], height: "100%"}}>
                    ...
                    <div style={{gap: layers.ref.spacing['6']}}>
                        <div data-testid="m3-card" style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-low)]est/50, borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['6'], border: "1px solid layers.sys.colors.outline"}}>
                            <h2  style={{marginBottom: layers.ref.spacing['8'], display: "flex", alignItems: "center", gap: layers.ref.spacing['8']}}>
                                <span  style={{color: "layers.sys.colors.primary"}}>monitoring</span>
                                Andamento Didattico
                            </h2>
                            <div style={{display: "flex", alignItems: "center", gap: layers.ref.spacing['6'], marginBottom: layers.ref.spacing['6']}}>
                                <div style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)], borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['8'], flex: "1", border: "1px solid layers.sys.colors.outline"}}>
                                    <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ display: "block", fontSize: "0.875rem", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: "bold" }}>Media Generale</span>
                                    <span className={`text-4xl font-bold ${parseFloat(performance.grade || '0') < 6 ? 'text-error' : 'text-primary'}`}>
                                        {performance.grade || '-'}
                                    </span>
                                </div>
                                <div style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)], borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['8'], flex: "1", border: "1px solid layers.sys.colors.outline"}}>
                                    <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ display: "block", fontSize: "0.875rem", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: "bold" }}>Trend</span>
                        ...
                        <div data-testid="m3-card" style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-low)]est/50, borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['6'], border: "1px solid layers.sys.colors.outline"}}>
                            <h2  style={{marginBottom: layers.ref.spacing['8'], display: "flex", alignItems: "center", gap: layers.ref.spacing['8']}}>
                                <span  style={{color: "layers.sys.colors.secondary"}}>history</span>
                                Ultime Valutazioni
                            </h2>
                            <div style={{gap: layers.ref.spacing['2']}}>
                                {evaluations.sort((a, b) => new Date(b.data).getTime() - new Date(a.data).getTime()).slice(0, 5).map(ev => (
                                    <div key={ev.id} style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-low)]est, borderRadius: ref.shape[] }} style={{display: "flex", justifyContent: "space-between", alignItems: "center", padding: layers.ref.spacing['6'], border: "1px solid layers.sys.colors.outline", transition: "color 300ms"}}>
                                        <div>
                                            <p style={{ color: sys.colors.[var(--md-sys-color-on-surface)] }} style={{ fontWeight: "bold" }}>{ev.materia}</p>
                                            <p style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ fontSize: "0.75rem" }}>{new Date(ev.data).toLocaleDateString()}</p>
                                        </div>
                    ...
                    <div style={{gap: layers.ref.spacing['6']}}>
                        <div data-testid="m3-card" style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-low)]est/50, borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['6'], border: "1px solid layers.sys.colors.outline"}}>
                            <h2  style={{marginBottom: layers.ref.spacing['8'], display: "flex", alignItems: "center", gap: layers.ref.spacing['8']}}>
                                <span  style={{color: "layers.sys.colors.tertiary"}}>verified</span>
                                Competenze Trasversali
                            </h2>
                            <div style={{gap: layers.ref.spacing['3']}}>
                                {recentCompetencies.length > 0 ? (
                                    recentCompetencies.map((comp: ValutazioneCompetenza, idx) => (
                                        <div key={idx} style={{ backgroundColor: sys.colors.tertiary-container/10, borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['8'], border: "1px solid layers.sys.colors.outline"}}>
                                            <div style={{display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: layers.ref.spacing['4']}}>
                                                <h4 style={{ color: sys.colors.[var(--md-sys-color-on-surface)] }} style={{ fontWeight: "bold" }}>{comp.name}</h4>
                                                <span style={{ color: sys.colors.on-tertiary }} style={{backgroundColor: "layers.sys.colors.tertiary", border: "none"}}>{comp.level}</span>
                                            </div>
                                            <p style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ fontSize: "0.875rem" }}>{comp.desc}</p>
                                            <p style={{ color: sys.colors.[10px], color: sys.colors.[var(--md-sys-color-on-surface)]-variant/60 }} style={{marginTop: layers.ref.spacing['4'], textTransform: "uppercase", letterSpacing: "0.1em"}}>Rilevato il {new Date(comp.date).toLocaleDateString()}</p>
                                        </div>
                                    ))
                                ) : (
                                    <InfoCard 
                        ...
                        <div data-testid="m3-card" style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-low)]est/50, borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['6'], border: "1px solid layers.sys.colors.outline"}}>
                            ...
                            <div  style={{display: "grid", gridTemplateColumns: "1fr", gap: layers.ref.spacing['8']}}>
                                <div style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)], borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['6']}}>
                                    <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ fontSize: "0.75rem", textTransform: "uppercase", fontWeight: "bold" }}>Classe</span>
                                    <p style={{ fontSize: "1.125rem", fontWeight: "bold" }}>{student.classe}</p>
                                </div>
                                <div style={{ backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)], borderRadius: ref.shape[] }} style={{padding: layers.ref.spacing['6']}}>
                                    <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ fontSize: "0.75rem", textTransform: "uppercase", fontWeight: "bold" }}>Bisogni</span>
                                    <div style={{display: "flex", gap: layers.ref.spacing['4'], marginTop: layers.ref.spacing['4']}}>
                                        {student.hasBES && <span style={{ backgroundColor: sys.colors.warning }} style={{ width: "0.75rem", height: "0.75rem", borderRadius: ref.spacing[9999] }} title="BES"></span>}
                                        {student.hasDSA && <span style={{ backgroundColor: sys.colors.error }} style={{ width: "0.75rem", height: "0.75rem", borderRadius: ref.spacing[9999] }} title="DSA"></span>}
                                        {student.has104 && <span style={{width: "0.75rem", height: "0.75rem", borderRadius: ref.spacing[9999], backgroundColor: "layers.sys.colors.primary"}} title="L.104"></span>}
