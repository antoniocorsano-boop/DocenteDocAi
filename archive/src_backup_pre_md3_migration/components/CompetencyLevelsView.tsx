        ...
        <div className={`level-card ${cardClass}`}>
            ...
            <div style={{ flexGrow: "1" }}>
                <div style={{display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: layers.ref.spacing['4']}}>
                    <h3 style={{ color: sys.colors.[var(--md-sys-typescale-headline-small)] }} style={{ fontWeight: "bold" }}>{nome}</h3>
                    <span style={{ backgroundColor: sys.colors.surface/50 }} style={{paddingLeft: layers.ref.spacing['4'], paddingRight: layers.ref.spacing['4'], borderRadius: "0.375rem"}}>Valore: {voto}</span>
                </div>
                <p style={{ color: sys.colors.[var(--md-sys-typescale-body-medium)] }} style={{ opacity: "0.9" }}>{descrizione}</p>
            </div>
        </div>
    );
};
        ...
        <div  style={{gap: layers.ref.spacing['6'], padding: layers.ref.spacing['8']}}>
            ...
            {competenze.map(competenza => (
                <div key={competenza.id} >
                    <div  style={{marginBottom: layers.ref.spacing['8'], borderBottom: "1px solid layers.sys.colors.outline"}}>
                        <h2 style={{ color: sys.colors.[var(--md-sys-typescale-headline-small)] }} style={{color: "layers.sys.colors.primary"}}>{competenza.nome}</h2>
                        <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant, backgroundColor: sys.colors.[var(--md-sys-color-surface-container-high)] }} style={{paddingLeft: layers.ref.spacing['4'], paddingRight: layers.ref.spacing['4'], borderRadius: "0.375rem"}}>
