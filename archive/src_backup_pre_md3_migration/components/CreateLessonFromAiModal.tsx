                    ...
                    <M3DialogContent >
                        ...
                        <div>
                            <div  style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <label  style={{color: "layers.sys.colors.primary", textTransform: "uppercase"}}>Obiettivi</label>
                                {matchingCurriculum && (
                                    <M3Button 
                                        type="button" 
                                        onClick={() => setIsObjectivePickerOpen(true)}
                                        variant="tonal"
                                         style={{ textTransform: "uppercase", letterSpacing: "0.1em", borderRadius: ref.spacing[9999] }}
                            ...
                            {matchingCurriculum && !obiettivi && (
                                <p  style={{fontSize: "0.75rem", color: "layers.sys.colors.primary", marginTop: layers.ref.spacing['4'], display: "flex", alignItems: "center", gap: layers.ref.spacing['8'], fontWeight: "bold", cursor: "pointer"}} onClick={() => setIsObjectivePickerOpen(true)}>
                        ...
                        {slots && availableSlots.length > 0 && (
                            <div style={{ backgroundColor: sys.colors.secondary-container/10, padding: ref.spacing[12], borderRadius: ref.shape[4] }} style={{border: "1px solid layers.sys.colors.outline", gap: layers.ref.spacing['6']}}>
                                <label style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ fontWeight: "900", textTransform: "uppercase" }}>Pianificazione Rapida (Opzionale)</label>
                                <div  style={{ display: "flex", flexWrap: "wrap" }}>
                                    {availableSlots.map(([key, slot]) => (
                                        ...
                                        >
                                            {selectedSlotKey === key && <span  style={{ fontSize: ref.spacing[16] }}>check</span>}
                        ...
                        <div>
                            <div  style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <label  style={{color: "layers.sys.colors.primary", fontWeight: "900", textTransform: "uppercase"}}>Adattamenti per l'Inclusività</label>
                                <M3Button 
                                    type="button" 
                                    onClick={handleGenerateAdaptations} 
                                    disabled={isAdaptationsLoading} 
                                    variant="text"
                                    style={{ borderRadius: ref.shape[] }} style={{display: "flex", alignItems: "center", gap: layers.ref.spacing['8'], fontWeight: "900", textTransform: "uppercase", fontSize: "0.75rem", transition: "all 300ms cubic-bezier(0.4, 0, 0.2, 1)"}}
                                ...
                                >
                                    {isAdaptationsLoading ? (
                                        ...
                                        <span  style={{ fontSize: ref.spacing[16] }}>auto_awesome</span>
                                    )}
            ...
            {isObjectivePickerOpen && matchingCurriculum && (
                ...
                >
                    <M3DialogContent style={{marginTop: layers.ref.spacing['8']}}>
                        <p  style={{color: "layers.sys.colors.primary", textTransform: "uppercase"}}>{matchingCurriculum.subject} - {matchingCurriculum.gradeLevel}</p>
                        {matchingCurriculum.nuclei.map(nucleo => (
                            <details key={nucleo.id}  open>
                                <summary >
                                    <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)] }} style={{ fontWeight: "900" }}>{nucleo.title}</span>
                                    <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)]-variant }} style={{ fontSize: "0.875rem" }}>expand_more</span>
                                </summary>
                                <div style={{padding: layers.ref.spacing['8'], gap: layers.ref.spacing['3'], backgroundColor: "layers.sys.colors.surface"}}>
                                    {nucleo.objectives.map(obj => (
                                        <button 
                                            key={obj.id}
                                            type="button"
                                            onClick={() => handleAddObjective(obj.text)}
                                            style={{ borderRadius: ref.shape[] }} style={{width: "100%", textAlign: "left", padding: layers.ref.spacing['6'], transition: "color 300ms", display: "flex", alignItems: "flex-start", gap: layers.ref.spacing['6']}}
                                        >
                                            <span  style={{color: "layers.sys.colors.primary", fontSize: ref.spacing[16], transition: "transform 300ms"}}>add_circle</span>
                                            <span style={{ color: sys.colors.[var(--md-sys-color-on-surface)] }} style={{ fontSize: "0.875rem", fontWeight: "500" }}>{obj.text}</span>
                                        </button>
                                    ))}
                                </div>
                            </details>
